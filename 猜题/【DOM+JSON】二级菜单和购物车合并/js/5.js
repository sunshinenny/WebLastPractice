window.onload = function createTable() {     //创建表格
	    
	var json =         '[{"全选":"", "商铺名":"苹果旗舰店","商品名称":"iPhone X" ,"单价":"¥8000","数量":1, "小计":"¥0"},{"全选":"", "商铺名":"", "商品名称":"iPhone 8","单价":"¥5500","数量":1, "小计":"¥0"},{"全选":"", "商铺名":"小米旗舰店","商品名称":"MIX2S" ,"单价":"¥4000","数量":1, "小计":"¥0"}, {"全选":"", "商铺名":"","商品名称":"小爱" ,"单价":"¥200","数量":1, "小计":"¥0"}]';    
	var emps = eval(json);     //  console.log(emps);
	     //创建table
	    
	var table = document.createElement("table");     //创建thead
	    
	...
	     //遍历emps中第1个对象的每个属性
	    
	for(var key in emps[0]) {         //创建th
		        
		var th = document.createElement("th");         //设置th的内容为当前属性名
		        
		th.innerHTML = key;        
	...
	    
	var tbody = document.createElement("tbody");     //将tbody追加到table下
	    
	table.appendChild(tbody);     //遍历emps中每个员工对象
	    
	for(var i = 0; i < emps.length + 1; i++) {         //创建一个tr
		        
		var tr = document.createElement("tr");         //将tr追加到tbody下
		        
		tbody.appendChild(tr);         //遍历当前员工的每个属性
		        
		for(var key in emps[i]) {              //创建一个td
			            
			var td = document.createElement("td");             //将td追加到tr中
			            
			tr.appendChild(td);             //设置td的内容为当前属性值
			            
	...
	total.innerHTML = "¥0";      //将table追加id为data的元素下
	    
	document.getElementById("data")        .appendChild(table);     
	addItem();     
	function addItem() {         //增加全选按钮
		        
		var selAll = document.querySelector("thead th:first-child");        
		var selectAll = "<input type=\"checkbox\"onclick=\"selectAll(this)\">";
		var toggleAll = "<button id=\"hideAll\" onclick=\"toggleAll(this)\">v</button>";         
		var a = "全选";        
		selAll.innerHTML = toggleAll + selectAll + a;         //加入店铺的选择按钮     
		        
		...
		for(var i = 0; i < thirdGoods.length; i++) {             //          if(i==thirdGoods.length-1) continue;
			            
		...
		for(var i = 0; i < nums.length; i++) {             //          if(i==nums.length-1) continue;
			            
...
function calc(btn) {     //1、实现点击加减号span中的数字随着变化
	     //获取btn的内容
	     //获取span的内容
	     //如果是加号，就把span中的内容加1
	...
	var total = n * price;     
	btn.parentNode.nextElementSibling.innerHTML = "¥" + total.toFixed(2);     //3、实现total 总计
	     //获取总计所造标签td的内容 去掉人民币符号  转化为浮点数
	     //"tfoot tr td:last-child" tfoot ->tr->td->last-child用来确定每一行最后一个元素的位置“就是小计”
	    
	var tfoot_td = document.querySelector("tfoot tr td:last-child");     //以上是红框小计
	     //以下是绿框统计
	    
	var chbs = document.querySelectorAll("input[name='adminID']");     //遍历chbs中每个chb
	    
	var tfoot_td = document.querySelector("tfoot tr td:last-child");     //设置td的内容为  人民币符号  +计算total加上price保留两位小数
...
function selectAll(chb) {     //查找所有input中name为admin的所有元素，即商铺元素；
	    
	var chbs = document.querySelectorAll("input[name='admin']");     //查找所有input中name为adminID的所有元素，即商品元素；
	    
	var chbs1 = document.querySelectorAll("input[name='adminID']");     //console.dir(chbs.length);
	...
	var listTotal = document.querySelectorAll("tbody tr td:last-child");     //每次点击该按钮重置alltotal为零；  
	    
	var alltotal = 0;    
	for(var i = 0; i < listTotal.length; i++) {        
		alltotal += parseFloat(listTotal[i].innerHTML.slice(1));    
...
function check(chb) {    
	...
	    
		...
		        
		var flag = 0;         //简单考虑，只要存在没有被选中，更改参数，全选不被选中；
		        
		for(var i = 0; i < chbs.length; i++) {            
			if(chbs[i].checked == false) flag = 1;        
		}        
		if(flag == 0)             chbAll.checked = true;        
		console.dir(flag);    
	}     //总消费
	    
	var tfoot_td = document.querySelector("tfoot tr td:last-child");    
	...
	if(chb.checked == false) {        
		...
		for(var i = 0; i < chbs1.length; i++) {            
			if(chbs1[i].checked == false) {                
				if(i < 2)                     chbs[0].checked = false;                
				else                     chbs[1].checked = false;            
			}        
		}    
	}     //单击单个按钮，他不在全选状态
	    
		...
		if(flag == 0)             chbs[0].checked = true;         //第二商铺的选中
		        
		for(var i = 2; i < 4; i++) {            
			if(chbs1[i].checked == false) flag1 = 1;        
		}        
		if(flag1 == 0)             chbs[1].checked = true;         //全选的选中 
		        
		for(var i = 0; i < chbs.length; i++) {            
			if(chbs[i].checked == false) flag2 = 1;        
		}        
		if(flag2 == 0)             chbAll.checked = true;    
	}     //计算总消费
	    
	var tfoot_td = document.querySelector("tfoot tr td:last-child");     //设置td的内容为  人民币符号  +计算total加上price保留两位小数
	     //var alltotal=parseFloat(tfoot_td.innerHTML.slice(1))+(btn.innerHTML=="+"?price:(-price));
